...
public class BasicInkExample : MonoBehaviour {
	...
	void CreateContentView (string text) {
        ...
        storyText.CrossFadeAlpha(1, .5f, true);
	}
	...
	Button CreateChoiceView (string text) {
        ...
        choiceGraphic.CrossFadeAlpha(1, .5f, true);
	...
	void RemoveChildren () {
        ...
        {
            t.CrossFadeAlpha(0, .5f, true);
        }

        Graphic[] gfx = canvas.transform.GetComponentsInChildren<Graphic>();
        foreach(Graphic g in gfx)
        {
            g.CrossFadeAlpha(0, .5f, true);
        }

        int childCount = canvas.transform.childCount;

        for (int i = childCount - 1; i >= 0; --i) {
			GameObject.Destroy (canvas.transform.GetChild (i).gameObject,.5f);
		}
	}
